...
if dataset_name == "Math23K":
    ...
    var_nums = ['x', 'y']
    ckpt_dir = "ALG514_b2s_mp_new_retrain5"
...
for fold in range(fold_num):
    ...
    for epoch in range(n_epochs):
        ...
        stage1_sentence_ids_batches = batches_dict['stage1_sentence_ids_batches']
        attention_mask_sentence_batches = batches_dict['attention_mask_sentence_batches']
        stage1_span_lengths = batches_dict['stage1_span_lengths']
        sentence_length_batches = batches_dict['sentence_length_batches']
        ...
        for idx in range(len(input_lengths)):
            ...
            for step in range(grad_acc_steps):
                ...
                loss, stage1_loss = train_lm2seq_v2(input_batches[idx][start_idx:end_idx],
                                    ...
                                    id_batches[idx][start_idx:end_idx], 
                                    copy_nums, generate_num_ids, stage1_encoder, encoder, decoder, 
                                    stage1_encoder_optimizer, encoder_optimizer, decoder_optimizer,
                                    ...
                                    beam_search=beam_search, var_nums=var_num_ids, labels_num=num_labels)
...
#             tokens_dict_train = bert_tokenizer(' '.join(train_batch['input_cell']), add_special_tokens=False)
#             raw_input_ids_train = tokens_dict_train["input_ids"]
#             tokens_dict_train["input_ids"] = []
#             stage1_sentence_ids_train = []
#             sentence_length_train = []
#             sentence_m_train = [list(group) for k, group in groupby(raw_input_ids_train, lambda x: x == 102 or
#                                                              x == 0) if not k]
            
#             for sentence_id, i in enumerate(sentence_m_train):
#                     sentence_length_train.append(len(i)+1)
#                     stage1_sentence_ids_train.extend([sentence_id] * (len(i)+1))
            
#             stage1_sentence_ids_train.extend([0] * (raw_input_ids_train.count(0)))
            
#             for t_id in raw_input_ids_train:
#                 if t_id == len(bert_tokenizer.vocab):
#                     tokens_dict_train["input_ids"].append(1)
#                 else:
#                     tokens_dict_train["input_ids"].append(t_id)
            
#             num_pos_train = []
#             for idx, t_id in enumerate(tokens_dict_train["input_ids"]):
#                 if t_id == 1:
#                     num_pos_train.append(idx)
            
#             attention_mask_sentence_train = [float(i != -100) for i in train_batch['stage1_span']]
#             train_res = evaluate_lm2seq_v2(tokens_dict_train["input_ids"], len(tokens_dict_train["input_ids"]),
#                                        tokens_dict_train["attention_mask"], tokens_dict_train["token_type_ids"], 
#                                        train_batch['stage1_span'], train_batch['stage1_span_len'],
#                                        stage1_sentence_ids_train, attention_mask_sentence_train,
#                                        sentence_length_train, train_batch["nums"], copy_nums, generate_num_ids,
#                                        stage1_encoder, encoder, decoder,
#                                        output_lang, beam_size=beam_size, beam_search=beam_search,
#                                        max_length=MAX_OUTPUT_LENGTH, var_nums=var_num_ids, labels_num=num_labels)
            
#             import traceback
#             try:
#                 train_val_ac, train_equ_ac, train_ans_ac, \
#                 train_res_result, train_tar_result = compute_equations_result(train_res,
#                                                                             train_batch["output_cell"],
#                                                                             output_lang,
#                                                                             train_batch["nums"],
#                                                                             train_batch['num_stack'],
#                                                                             ans_list=train_batch['ans'],
#                                                                             tree=False, prefix=False)
#             except Exception as e:
#                 train_val_ac, train_equ_ac, train_ans_ac = False, False, False
...
#         logs_content = "{}, {}, {}".format(train_equation_ac, train_value_ac, train_total)
#         add_log(log_file, logs_content)
#         logs_content = "train_answer_acc: {} {}".format(float(train_equation_ac) / train_total, float(train_value_ac) / train_total)
#         add_log(log_file, logs_content)
#         logs_content = "train eval time: {}".format(time_since(time.time() - start))
#         add_log(log_file, logs_content)
#         logs_content = "------------------------------------------------------"
#         add_log(log_file, logs_content)
#         all_acc_data_train.append((fold, epoch, train_equation_ac, train_value_ac, train_total))
        ...
        if epoch % 1 == 0 or epoch > n_epochs - 5:
            ...
            for test_batch in test_pairs:
                ...
                #         num_pos.append(idx)
                #stage1_len_max = max(test_batch['stage1_span_len'])
                #stage1_span_ids = pad_seq(test_batch['stage1_span'], test_batch['stage1_span_len'], 
                #                          stage1_len_max, pad_token=-100)
                #print(tokens_dict["attention_mask"])
                #print(test_batch['stage1_span'])
                #print(stage1_sentence_ids)
                #print(attention_mask_sentence)

                attention_mask_sentence = [float(i != -100) for i in test_batch['stage1_span']]
                print(test_batch['id'])
                test_res = evaluate_lm2seq_v2(tokens_dict["input_ids"], len(tokens_dict["input_ids"]),
                                           tokens_dict["attention_mask"], tokens_dict["token_type_ids"], 
                                           test_batch['stage1_span'],
                                           test_batch['stage1_span_len'], stage1_sentence_ids, attention_mask_sentence,
                                           sentence_length, test_batch["nums"], copy_nums, generate_num_ids,
                                           stage1_encoder, encoder, decoder,
                                           output_lang, beam_size=beam_size, beam_search=beam_search,
                                           max_length=MAX_OUTPUT_LENGTH, var_nums=var_num_ids, labels_num=num_labels)
            ...
            torch.save(stage1_encoder.state_dict(), os.path.join(current_save_dir, "lm2seq_stage1_encoder"))
            ...
            if best_val_acc < value_ac:
                ...
                #current_best_train_acc = (train_equation_ac, train_value_ac, train_total)
                torch.save(stage1_encoder.state_dict(), os.path.join(current_save_dir, "lm2seq_stage1_encoder_best_val_acc"))
            ...
            if best_equ_acc < equation_ac:
                ...
                torch.save(stage1_encoder.state_dict(), os.path.join(current_save_dir, "lm2seq_stage1_encoder_best_equ_acc"))
            ...
            if epoch == n_epochs - 1:
                ...
                #last_train_acc_fold.append((train_equation_ac, train_value_ac, train_total))
                #best_train_acc_fold.append(current_best_train_acc)

# a, b, c = 0, 0, 0
# for bl in range(len(last_train_acc_fold)):
#     a += last_train_acc_fold[bl][0]
#     b += last_train_acc_fold[bl][1]
#     c += last_train_acc_fold[bl][2]
#     logs_content = "{}".format(last_train_acc_fold[bl])
#     add_log(log_file, logs_content)
...
# a, b, c = 0, 0, 0
# for bl in range(len(best_train_acc_fold)):
#     a += best_train_acc_fold[bl][0]
#     b += best_train_acc_fold[bl][1]
#     c += best_train_acc_fold[bl][2]
#     logs_content = "{}".format(best_train_acc_fold[bl])
#     add_log(log_file, logs_content)
# logs_content = "{} {}".format(a / float(c), b / float(c))
# add_log(log_file, logs_content)
# logs_content = "------------------------------------------------------"
...
#logs_content = "{}".format(all_acc_data_train)
#add_log(log_file, logs_content)
