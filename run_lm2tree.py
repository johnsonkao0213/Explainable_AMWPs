...
parser.add_argument('--embedding_size', type=int, default=128) #original: 128
...
if dataset_name == "Math23K":
    ...
    var_nums = ['x','y']
    ckpt_dir = "ALG514_b2t_equation_index_concat_cls_add_last_plus_early_fusion_prev_eq_prof5"
    bert_path = "bert-base-uncased"
    data_path = "./dataset/alg514/questions_normalization_v8.json"
...
for p in pairs:
    ...
    if p["prev_eq"]:
        ...
        #if p['id'] == 313:
        #    print(ept2.get_prefix_expression())
        #p['prev_eq'] = ept2.get_infix_expression()
    #if p['id'] == 313:
    #    print(p)
    temp_pairs.append(p)
pairs = temp_pairs
...
for fold in range(fold_num):
    ...
    eq1_encoder = Eq1_Encoder_v2(op_nums=output_lang.n_words - copy_nums - 1 - len(generate_nums) - len(var_nums),
                         input_size=len(generate_nums) + len(var_nums))
    ...
    for epoch in range(n_epochs):
        ...
        batch_dict = prepare_data_batch_origin(train_pairs, batch_size, lm_tokenizer=bert_tokenizer,
        ...
        prev_eq_num_pos_batches = batch_dict['prev_eq_num_pos_batches']
        ...
        for idx in range(len(input_lengths)):
            ...
            for step in range(grad_acc_steps):
                ...
                if step == 0:
                    ...
                    zero_grad = False
                #print(id_batches[idx][start_idx:end_idx])
                #print(prev_eq_batches[idx][start_idx:end_idx])
                #print(prev_eq_len_batches[idx][start_idx:end_idx])
                #print(prev_eq_num_pos_batches[idx][start_idx:end_idx])
                loss = train_lm2tree(input_batches[idx][start_idx:end_idx],
                                     ...
                                     prev_eq_batches[idx][start_idx:end_idx],
                                     prev_eq_len_batches[idx][start_idx:end_idx],
                                     prev_eq_num_pos_batches[idx][start_idx:end_idx],
                                     ...
                                     eq1_encoder_optimizer, output_lang,
...
#                             quantity_indicator.append(0)
                ...
                #prev_eq_num_pos = []
                prev_eq_num_pos = [0] * len(tokens_dict["input_ids"])
                for i in test_batch['prev_eq_num_pos']:
                ...
                if not test_batch['prev_eq']:
                    test_batch['prev_eq'] = [0]
                test_res = evaluate_lm2tree(tokens_dict["input_ids"], len(tokens_dict["input_ids"]), 
                                            ...
                                            test_batch['equation_index'], test_batch['var_cnt'], test_batch['prev_eq'], 
                                            len(test_batch['prev_eq']), prev_eq_num_pos, generate_num_ids, 
                                            encoder, predict, generate, merge, 
                                            eq1_encoder, output_lang, num_pos, beam_size=beam_size, beam_search=beam_search,
                                            var_nums=var_num_ids)
                ...
                if cnt == test_batch['var_cnt']:
                    ...
                    test_res_result, test_tar_result, equ11_ac, equ1_ac, equ2_ac = compute_equations_result_v2(test_res_list, 
                                                                                test_tar_list,
