...
if dataset_name == "Math23K":
    ...
    var_nums = ['x','y']
    ckpt_dir = "ALG514_b2t_equation_index_concat_cls_early_fusion_pipeline_update" #_plus_eq_embedding
    bert_path = "bert-base-uncased"
    data_path = "./dataset/alg514/questions_normalization_v9.json"
...
if dataset_name == "Math23K":
    ...
    data = load_alg514_data_bystep(data_path, stage1_path)
    print(data[0])
    pairs, generate_nums, copy_nums = transfer_alg514_num_bystep(data)
elif dataset_name == "mawps":
...
for p in pairs:
    ...
    if p["out_seq2"]:
        ept2 = ExpressionTree()
        ept2.build_tree_from_infix_expression(p["out_seq2"])
        p['out_seq2'] = ept2.get_prefix_expression()
#     if p["prev_eq"]:
#         ept2 = ExpressionTree()
#         ept2.build_tree_from_infix_expression(p["prev_eq"])
#         p['prev_eq'] = ept2.get_prefix_expression()
    temp_pairs.append(p)
pairs = temp_pairs
print(pairs[0])
...
for fold in range(fold_num):
    ...
    _, output_lang, train_pairs, test_pairs = prepare_data_bystep(pairs_trained, pairs_tested, 5, generate_nums,
                                                        copy_nums, tree=True, use_lm=True, use_group_num=False)
    #print(train_pairs[0])
    embedding_weight = None

    # Initialize models
    encoder = PLMEncoderSeq_v4(model_path=bert_path)
    #encoder = PLMEncoderSeq_v5(model_path=bert_path, output_size=output_lang.n_words)
    ...
    eq1_encoder_optimizer = torch.optim.Adam(eq1_encoder.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    eq1_encoder_scheduler = torch.optim.lr_scheduler.StepLR(eq1_encoder_optimizer, step_size=max(n_epochs//4,1), gamma=0.5)
    ...
    for epoch in range(n_epochs):
        ...
        batch_dict = prepare_data_batch_origin_bystep(train_pairs, batch_size, lm_tokenizer=bert_tokenizer,
        ...
        output2_batches = batch_dict['output2_batches']
        output2_lengths = batch_dict['output2_lengths']
        nums_batches = batch_dict['nums_batches']
        num_stack_batches = batch_dict['num_stack_batches']
        num_stack2_batches = batch_dict['num_stack2_batches']
        ...
        stage1_span_ids2_batches = batch_dict['stage1_span_ids2_batches']
        ...
        attention_mask_sentence2_batches = batch_dict['attention_mask_sentence2_batches']
        cls_loc_batches = batch_dict['cls_loc_batches']
        cls_loc2_batches = batch_dict['cls_loc2_batches']
        var_cnt_batches = batch_dict['var_cnt_batches']
        equation_id_batches = batch_dict['equation_id_batches']
        equation_id2_batches = batch_dict['equation_id2_batches']
#         prev_eq_batches = batch_dict['prev_eq_batches']
#         prev_eq_len_batches = batch_dict['prev_eq_len_batches']
#         attention_mask_prev_eq_batches = batch_dict['attention_mask_prev_eq_batches']
#         print(id_batches[1][0:3])
#         print(input_batches[1][0:3])
#         print(input_lengths[1][0:3])
#         print(attention_mask_batches[1][0:3])
#         print(token_type_ids_batches[1][0:3])
#         print(output_batches[1][0:3])
#         print(output_lengths[1][0:3])
#         print(output2_batches[1][0:3])
#         print(output2_lengths[1][0:3])
#         print(nums_batches[1][0:3])
#         print(num_stack_batches[1][0:3])
#         print(num_stack2_batches[1][0:3])
#         print(num_pos_batches[1][0:3])
#         print(num_size_batches[1][0:3])
#         print(ans_batches[1][0:3])
#         print(stage1_span_ids_batches[1][0:3])
#         print(stage1_span_ids2_batches[1][0:3])
#         print(attention_mask_sentence_batches[1][0:3])
#         print(attention_mask_sentence2_batches[1][0:3])
#         print(cls_loc_batches[1][0:3])
#         print(cls_loc2_batches[1][0:3])
#         print(var_cnt_batches[1][0:3])
#         print(equation_id_batches[1][0:3])
#         print(equation_id2_batches[1][0:3])
        ...
        for idx in range(len(input_lengths)):
            ...
            for step in range(grad_acc_steps):
                ...
                loss = train_lm2tree_bystep(input_batches[idx][start_idx:end_idx],
                                     ...
                                     equation_id_batches[idx][start_idx:end_idx],
                                     var_cnt_batches[idx][start_idx:end_idx],
                                     stage1_span_ids2_batches[idx][start_idx:end_idx],
                                     output2_batches[idx][start_idx:end_idx],
                                     output2_lengths[idx][start_idx:end_idx],
                                     num_stack2_batches[idx][start_idx:end_idx],
                                     cls_loc2_batches[idx][start_idx:end_idx],
                                     attention_mask_sentence2_batches[idx][start_idx:end_idx],
                                     equation_id2_batches[idx][start_idx:end_idx],
                                     generate_num_ids, encoder, predict, generate, merge, eq1_encoder,
                                     encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, 
                                     eq1_encoder_optimizer, output_lang, num_pos_batches[idx][start_idx:end_idx], 
                                     id_batches[idx][start_idx:end_idx], use_clip=use_clip, clip=gclip,
        ...
        if epoch % 1 == 0 or epoch > n_epochs - 5:
            ...
            for test_batch in test_pairs:
                #if cnt != 0 and test_batch['id'] != tmp_id:
                #test_res_list = []
                test_tar_list = []
                num_stack_list = []
                    #cnt = 0
                ...
                for i, j in zip(sentence_m, test_batch['stage1_span2']):
                    stage1_span_ids2.extend([j] * (len(i)+1))
...
#                         if stage1_span_ids[idx] == 1:
#                             quantity_indicator.append(1)
#                         else:
#                             quantity_indicator.append(0)
                    elif t_id == 101 and stage1_span_ids[idx] == 1 and stage1_span_ids2[idx] == 1:
                    #elif t_id == 101:
                        cls_loc.append(idx)
                        cls_loc2.append(idx)
                        quantity_indicator.append(0)
                    elif t_id == 101 and stage1_span_ids[idx] == 1:
                    #elif t_id == 101:
                        cls_loc.append(idx)
                        quantity_indicator.append(0)
                    elif t_id == 101 and stage1_span_ids2[idx] == 1:
                    #elif t_id == 101:
                        cls_loc2.append(idx)
                        quantity_indicator.append(0)
                    else:
                ...
                cls_loc_padding = pad_seq(cls_loc, len(cls_loc), 9, pad_token=-1)
                attention_mask_sentence = [float(i != -1) for i in cls_loc_padding]
                cls_loc2_padding = pad_seq(cls_loc2, len(cls_loc2), 9, pad_token=-1)
                attention_mask_sentence2 = [float(i != -1) for i in cls_loc2_padding]
                test_res = evaluate_lm2tree_bystep(tokens_dict["input_ids"], len(tokens_dict["input_ids"]), 
                                            tokens_dict["attention_mask"], tokens_dict["token_type_ids"], stage1_span_ids, 
                                            stage1_sentence_ids, quantity_indicator, cls_loc_padding, attention_mask_sentence, 
                                            test_batch['equation_index'], test_batch['var_cnt'], stage1_span_ids2,
                                            cls_loc2_padding, attention_mask_sentence2, test_batch['equation_index2'],
                                            generate_num_ids, encoder, predict, generate, merge, eq1_encoder,
                                            output_lang, num_pos, beam_size=beam_size, beam_search=beam_search,
                                            var_nums=var_num_ids)
                #test_res_list.append(test_res)
                test_tar_list.append(test_batch['output_cell'])
                num_stack_list.append(test_batch['num_stack'])
                #test_res_list += test_res
                #cnt += 1
                #tmp_id = test_batch['id']
                #if cnt == test_batch['var_cnt']:
                if test_batch['var_cnt'] == 2:
                    test_tar_list.append(test_batch['output_cell2'])
                    num_stack_list.append(test_batch['num_stack2'])
                    
                val_ac, equ_ac, ans_ac, \
                test_res_result, test_tar_result = compute_equations_result_v2(test_res, 
                                                                            test_tar_list,
                                                                            output_lang, 
                                                                            test_batch['nums'],
                                                                            num_stack_list,
                ...
                eval_total += 1
                    ...
                    #import traceback
                    #try:
#                         if test_batch['var_cnt'] == 1:
#                             val_ac, equ_ac, ans_ac, \
#                             test_res_result, test_tar_result = compute_equations_result(test_res_list, 
#                                                                                         test_tar_list,
...
#                                                                                         tree=True, prefix=True)
            ...
            #torch.save(eq1_encoder.state_dict(), os.path.join(current_save_dir, "seq2tree_eq1_encoder"))
            if best_val_acc < value_ac:
                ...
                #torch.save(eq1_encoder.state_dict(), os.path.join(current_save_dir, "seq2tree_eq1_encoder_best_val_acc"))
            if best_equ_acc < equation_ac:
                ...
                #torch.save(eq1_encoder.state_dict(), os.path.join(current_save_dir, "seq2tree_eq1_encoder_best_equ_acc"))
            if epoch == n_epochs - 1:
