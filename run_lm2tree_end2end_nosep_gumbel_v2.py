...
if dataset_name == "Math23K":
    ...
    var_nums = ['x','y']
    ckpt_dir = "ALG514_b2t_nosep_gumbel_13"
...
for fold in range(fold_num):
    ...
    # Initialize models
    #stage1_encoder = Stage1_Encoder_nosep_v2(num_labels, bert_tokenizer, use_sentence_index, aggregate_mode, rnn_type)
    #encoder = PLMEncoderSeq_v3(model_path=bert_path)
    encoder = Stage1_Encoder_nosep_v3(num_labels, bert_tokenizer, use_sentence_index, aggregate_mode, rnn_type)
    ...
    for epoch in range(n_epochs):
        ...
        batch_dict = prepare_data_batch_nosep(train_pairs, batch_size, lm_tokenizer=bert_tokenizer,
        ...
        for idx in range(len(input_lengths)):
            ...
            for step in range(grad_acc_steps):
                ...
                #
                loss, stage1_loss, sent_acc, pro_acc, problem_len = train_lm2tree_v4(input_batches[idx][start_idx:end_idx],
        ...
        if epoch % 1 == 0 or epoch > n_epochs - 5:
            ...
            for test_batch in test_pairs:
                ...
                raw_attention_mask = tokens_dict["attention_mask"]
                raw_token_type_ids = tokens_dict["token_type_ids"]
                ...
                for sentence_id, i in enumerate(sentence_m):
                    ...
                    stage1_sentence_ids.extend([sentence_id] * len(i))
                    
                sentence_length[-1] += 1
                stage1_sentence_ids.extend([len(sentence_m)-1])
                stage1_sentence_ids.extend([0] * (raw_input_ids.count(0)))
                sep_count = raw_input_ids.count(102)
                cnt = 1
                for t_id, t_mask, t_type_id in zip(raw_input_ids, raw_attention_mask, raw_token_type_ids):
                    if t_id == len(bert_tokenizer.vocab):
                        tokens_dict["input_ids"].append(1)
                        tokens_dict["attention_mask"].append(t_mask)
                        tokens_dict["token_type_ids"].append(t_type_id)
                ...
                #print(test_batch['id'])
                #quantity_indicator
                test_res, test_sent_acc, test_pro_acc, test_problem_len = evaluate_lm2tree_v4(tokens_dict["input_ids"], 
                                               ...
                                               quantity_indicator, generate_num_ids, 
