    ...
    while idx < len(test_expression):
        if test_expression[idx] == '**' and idx + 1 < len(test_expression):
            try:
                ...
                ip = int(p)
                if p > 3.0: #  or p - ip != 0: 放宽对指数的限制
                # if p > 3.0 or (p - ip != 0 and p - ip != 0.5):
                    return False, False, False, test_expression, tar_expression
            except:
                return False, False, False, test_expression, tar_expression
        idx = idx + 1
...
#@timeout_decorator.timeout(1)
def compute_equations_result_v2(test_res_list, test_tar_list, output_lang, num_list, num_stack, var_cnt, 
                             ans_list=[], tree=False, prefix=False, infix=False, postfix=False):
    if not isinstance(test_res_list, list):
        test_res = test_res.tolist()
    
    test_expression_list = []
    tar_expression_list = []
    #equ1_ac, equ11_ac, equ2_ac = False, False, False
    for i in range(var_cnt):
#         if len(num_stack) == 0 and test_res_list == test_tar_list:
#             test = out_expression_list(test_res_list[i], output_lang, num_list)
#             tar = out_expression_list(test_tar_list[i], output_lang, num_list, copy.deepcopy(num_stack))
#             if not tree or (tree and infix):
#                 test_ept = ExpressionTree()
#                 test_ept.build_tree_from_infix_expression(test)
#                 tar_ept = ExpressionTree()
...
#             return True, True, True, test_expression, tar_expression
            # return True, True, True, test_res, test_tar
        test = out_expression_list(test_res_list[i], output_lang, num_list)
        tar = out_expression_list(test_tar_list[i], output_lang, num_list, copy.deepcopy(num_stack[i]))
        if test is None:
            if not tree or (tree and infix):
                tar_ept = ExpressionTree()
                tar_ept.build_tree_from_infix_expression(tar)
            ...
            if not tree or (tree and infix):
                ...
                tar_expression = tar_ept.get_infix_expression()
            return False, False, False, test, tar_expression, False, False, False
            # return False, False, False, test, tar
        alphabet = "abcdefghijklmnopqrstuvwxyz"
        test_var_list = set()
        for elem in test:
        ...
        test_expression = test_ept.get_infix_expression()
        if test_expression == []:
            return False, False, False, test_expression, tar_expression, False, False, False
        #print(test_expression)
        #print(tar_expression)
        new_tar_expression = []
        for elem in tar_expression:
...
#         if i == 0:
#             if var_cnt == 2:
#                 equ1_ac = compute_equation_acc(test_expression, tar_expression)
#             else:
#                 equ11_ac = compute_equation_acc(test_expression, tar_expression)
#         else:
#             equ2_ac = compute_equation_acc(test_expression, tar_expression)
        test_expression_list += test_expression
        test_expression_list += ['[SEP]']
        tar_expression_list += tar_expression
        tar_expression_list += ['[SEP]']
    ...
    del test_expression_list[-1]
    del tar_expression_list[-1]
#     print(test_expression_list)
#     print(tar_expression_list)
    test_ans = compute_expressions(test_expression_list, test_var_list)
    tar_ans = compute_expressions(tar_expression_list, tar_var_list)

    equ_ac, equ11_ac, equ1_ac, equ2_ac = compute_equation_acc(test_expression_list, tar_expression_list)
    if test_ans == "error" or test_ans == []:
        return False, False, False, test_expression_list, tar_expression_list, equ11_ac, equ1_ac, equ2_ac
    val_ac = True
    #equ_ac = True
    ans_ac = True
    
#     if test_expression_list == tar_expression_list:
#         equ_ac = True
#     else:
#         equ_ac = False
    ...
    if len(ans_list) == 0:
        ...
        if change:
            ans_ac = False
    return val_ac, equ_ac, ans_ac, test_expression_list, tar_expression_list, equ11_ac, equ1_ac, equ2_ac


def compute_equation_acc(test_expression, tar_expression):
    test_expression_str = ' '.join(test_expression)
    test_expression_strs = test_expression_str.split('[SEP]')
    tar_expression_str = ' '.join(tar_expression)
    tar_expression_strs = tar_expression_str.split('[SEP]')
    test_equ_list = []
    tar_equ_list = []
    for test_equ_str, tar_equ_str in zip(test_expression_strs, tar_expression_strs):
        test_equ_str_split = test_equ_str.split('=')
        tar_equ_str_split = tar_equ_str.split('=')
        if len(test_equ_str_split) == 1:
            test_expr = parse_expr(test_equ_str_split[0])
            test_equ_list.append(str(test_expr))
            #test_equ_list.append(str(parse_expr(test_equ_str_split[0])))
        else:
            test_expr = parse_expr(test_equ_str_split[0] + ' - ( ' + test_equ_str_split[1] + ' )')
            test_equ_list.append(str(test_expr))
        if len(tar_equ_str_split) == 1:
            tar_expr = parse_expr(tar_equ_str_split[0])
            tar_equ_list.append(str(tar_expr))
        else:
            tar_expr = parse_expr(tar_equ_str_split[0] + ' - ( ' + tar_equ_str_split[1] + ' )')
            if (test_expr * -1) == tar_expr:
                tar_expr = tar_expr * -1
            tar_equ_list.append(str(tar_expr))
    
    equ11_ac, equ1_ac, equ2_ac = False, False, False
    equ_ac = False
    if set(test_equ_list) == set(tar_equ_list):
        equ_ac = True
    if len(tar_equ_list) == 1:
        if test_equ_list[0] == tar_equ_list[0]:
            equ11_ac = True
    else:
        if test_equ_list[0] == tar_equ_list[0]:
            equ1_ac = True
        if test_equ_list[1] == tar_equ_list[1]:
            equ2_ac = True
    
    return equ_ac, equ11_ac, equ1_ac, equ2_ac
